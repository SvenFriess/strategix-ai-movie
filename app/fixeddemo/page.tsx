...
/**
 * Filmfreundliche Q&A-Ansicht für feste Antworten (offline).
 * Tastatur:
 *  - N: Nächste Frage
 *  - P: Vorherige Frage
 *  - A / Enter: Antwort ein/aus
 *  - Cmd/Ctrl+C: Antwort in Zwischenablage
 */
export default function FixedDemoPage() {
  const keys = useMemo(() => Object.keys(FIXED_RESPONSES), []);
  const [idx, setIdx] = useState(0);
  const [showAnswer, setShowAnswer] = useState(false);
  const [fontScale, setFontScale] = useState(1); // Zoom für Filmaufnahme
  ...
  const copyAnswer = useCallback(async () => {
    try {
      ...
      // Ignorieren; in sehr restriktiven Browsern evtl. nicht erlaubt
    }
  }, [answer]);
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (key === "n" || key === "arrowright") {
        ...
        e.preventDefault();
        setFontScale((s) => Math.max(0.8, Number((s - 0.1).toFixed(2))));
      } else if (key === "0") {
        e.preventDefault();
        setFontScale(1);
      }
  ...
  // Inline-Styles (keine Tailwind-Abhängigkeit nötig)
  const styles = {
    page: {
      fontFamily: "system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif",
      background: "#0b0c10", // dunkler Hintergrund für Film
      color: "#eaf1f1",
      minHeight: "100vh",
      padding: "3rem 2rem",
      display: "flex",
  ...
  return (
    <main style={styles.page}>
      <div style={styles.wrap}>
        ...
        <p style={styles.sub}>
          {idx + 1} / {keys.length} · Taste <b>N</b> weiter, <b>P</b> zurück · <b>A/⏎</b>{" "}
          Antwort · <b>Cmd/Ctrl+C</b> kopieren
        </p>

        <div style={styles.grid}>
          {/* Liste linker Bereich */}
          ...
          <section>
            <div style={styles.card}>
              ...
              <div style={styles.buttons}>
                ...
                <button style={styles.btn} onClick={copyAnswer} title="In Zwischenablage kopieren">
                  ⧉ Kopieren (⌘/Ctrl+C)
                </button>
              </div>
            </div>
